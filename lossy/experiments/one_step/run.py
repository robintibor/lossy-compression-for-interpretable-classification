...
import sys
from lossy.image_convert import ImageConverter
...
def run_exp(output_dir, n_epochs, optim_type, n_start_filters, residual_preproc,
            lr, threshold, bpd_weight, np_th_seed, first_n, debug):
    tqdm = lambda x: x
    trange = range
    set_random_seeds(np_th_seed, True)
    zero_init_residual = False#True#False
    initialization = 'xavier_normal'#kaiming_normal
    restart_epochs = None#5,10,50,None
    weight_decay = 5e-5
    ...
    channel, im_size, num_classes, class_names, trainloader, train_det_loader, testloader = get_dataset(
        dataset, data_path, batch_size=batch_size, standardize=False,
        split_test_off_train=split_test_off_train, first_n=first_n)

    mean = [0.4914, 0.4822, 0.4465]
    std = [0.2023, 0.1994, 0.2010]
    normalize = kornia.augmentation.Normalize(mean=np_to_th(mean, device='cpu', dtype=np.float32),
                                              std=np_to_th(std, device='cpu', dtype=np.float32))
    nf_net = NFResNetCIFAR(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, activation='elu', alpha=0.2, beta=1.0,
                           zero_init_residual=zero_init_residual,
                           base_conv=ScaledStdConv2d, n_start_filters=n_start_filters,
    ...
    opt_preproc = torch.optim.Adam(preproc.parameters(),
                                   lr=1e-3, weight_decay=5e-5, betas=beta_preproc)
    if restart_epochs is not None:
        scheduler = CosineAnnealingWarmRestarts(opt_clf, T_0=len(trainloader) * restart_epochs)
    else:
        # no scheduling
        scheduler = LambdaLR(opt_clf, lr_lambda=lambda *args: 1)

    if n_warmup_epochs > 0:
        scheduler = WarmupBefore(scheduler, n_warmup_steps=len(trainloader) * n_warmup_epochs)
    if opt_clf.__class__.__name__ == 'AdamW':
    ...
    def get_aug_m(X_shape):
        noise = th.randn(*X_shape, device='cuda') * 0.01
        aug_m = nn.Sequential(
            FixedAugment(kornia.augmentation.RandomCrop((32, 32), padding=4, ), X_shape, ),
    ...
    if adjust_betas:
        ...
        with th.no_grad():
            init_X = th.cat([get_aug_m(X.shape)(X.cuda())
    ...
    image_converter = ImageConverter(image_standardize_before_glow=False, sigmoid_on_alpha=True,
    ...
    gen = nn.Sequential(
        Expression(img_0_1_to_glow_img),
        glow)
    ...
    for i_epoch in trange(n_epochs + n_warmup_epochs):
        for X, y in tqdm(trainloader):
            ...
            with higher.innerloop_ctx(clf, opt_clf, copy_initial_weights=True) as (f_clf, f_opt_clf):
            ...
            bpd_factor = bpd_weight * ((1 / threshold) * (threshold - f_orig_loss).clamp(0, 1)).detach()
            im_loss = weighted_sum(1, 1, f_simple_loss, 10, f_orig_loss,
                                   bpd_factor, bpd_loss)
        ...
        with nb_res.output_area('accs_losses'):
            ...
            with torch.no_grad():
                for with_preproc in [True, False]:
                    for set_name, loader in (('train', train_det_loader), ('test', testloader)):
                        all_preds = []
                        all_ys = []
                        all_losses = []
                        if with_preproc:
                        ...
                        for X, y in tqdm(loader):
                            ...
                            all_losses.append(th_to_np(th.nn.functional.cross_entropy(preds, y, reduction='none')))
    ...
    with th.no_grad():
        ...
        if with_preproc:
            X_preproced = preproc(X)
        th.save(X_preproced.detach().cpu(), os.path.join(output_dir, "X_preproced.th"), )
        save_image(X_preproced, os.path.join(output_dir, "X_preproced.png"), nrow=int(np.sqrt(len(X_preproced))))
