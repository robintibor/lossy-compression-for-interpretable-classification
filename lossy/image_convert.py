    ...
    def alpha_to_img_orig(self, alphas):
        if self.sigmoid_on_alpha:
            ...
            if self.glow_noise_on_out:
                im_orig = (
                    im_orig + torch.rand_like(im_orig) * 1 / 255.0
                )  # now onto [0,1+1/255.0=256/255.0], but later undone by to glow conversion
        else:
...
def min_max_normalize_per_example(X):
    X = X - torch.flatten(X, 1).min(dim=1, keepdim=True)[0].view(
        -1, *((1,) * (X.ndim - 1))
    )
    X = X / torch.flatten(X, 1).max(dim=1, keepdim=True)[0].view(
        -1, *((1,) * (X.ndim - 1))
    )
    return X
