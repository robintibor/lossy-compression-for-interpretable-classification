...
def get_dataset(dataset, data_path, standardize=True, split_test_off_train=False):
    if dataset == 'MNIST':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        ...
        transform = transforms.Compose([transforms.ToTensor(),
                                        transforms.Resize(32,),# transforms.InterpolationMode.BILINEAR),
                                        transforms.Lambda(lambda x: torch.cat((x,x,x), dim=0))])
        dst_train = datasets.USPS(data_path, train=True, download=False, transform=transform) # no augmentation
        dst_test = datasets.USPS(data_path, train=False, download=False, transform=transform)
        class_names = [str(c) for c in range(num_classes)]
    elif dataset == 'SVHN':
        channel = 3
        im_size = (32, 32)
...
def get_network(model, channel, num_classes, im_size=(32, 32), net_norm_override=None,
...
def epoch(mode, dataloader, net, optimizer, criterion, param_augment, device, image_converter):
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader,
                    learningrate, batchsize_train, param_augment, device, Epoch = 600, image_converter=None):
    net = net.to(device)
