...
def get_CIFAR100(augment, dataroot, download, first_n, reflect_pad, grey):
    image_shape = (32, 32, 3)
    num_classes = 100
    test_transformations = []
    if grey:
...
def get_SVHN(augment, dataroot, download, first_n, reflect_pad, grey):
    image_shape = (32, 32, 3)
    num_classes = 10
...
def load_train_test(dataset, shuffle_train, drop_last_train,
    ...
    if dataset == 'tiny':
        ...
        return load_tiny_imagenet(first_n=first_n, batch_size=batch_size,
            n_workers=n_workers, eval_batch_size=eval_batch_size,
            shuffle_train=shuffle_train, drop_last_train=drop_last_train)
    else:
...
def load_train_test_tiny(
        ...
        augment=False, grey=False, resize_tiny=None):
    ...
    if augment:
        if reflect_pad:
            translate = transforms.RandomCrop(size=32, padding=3, padding_mode='reflect')
        else:
            translate = transforms.RandomAffine(0, translate=(0.1, 0.1))
        train_image_transforms.extend([translate,
    ...
    if (len(train_image_transforms) + len(common_image_transforms)) > 0:
        train_transforms = [partial(to_tiny_image, grey=grey)] + train_image_transforms +  (
    ...
    if len(common_image_transforms) > 0:
        test_transforms = [partial(to_tiny_image, grey=grey)] + (
                common_image_transforms + [transforms.ToTensor(), preprocess])
    ...
    if augment:
        if reflect_pad:
            train_transformations.append(
                transforms.RandomCrop(size=32, padding=3, padding_mode='reflect'))
        else:
            train_transformations.append(
